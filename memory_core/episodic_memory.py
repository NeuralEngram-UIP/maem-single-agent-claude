...
def compute_importance(text: str) -> float:
    """
    Use LLM to score how important this memory is to retain.
    Returns float 0.0 to 1.0.
    """
    # Fast path — skip obvious low-value content
    if len(text.split()) <= 3:
        return 0.2
    ...
    try:
        resp = client.messages.create(
            ...
            max_tokens=5,
            system=(
                "Rate how important this statement is to remember about a user "
                "for future personalised conversations. "
                "Personal facts, preferences, goals, emotional content = HIGH. "
                "Small talk, greetings, generic questions = LOW. "
...
def _heuristic_importance(text: str) -> float:
    """Fallback only — used when LLM call fails."""
    text_lower = text.lower()

    high_signals = ["i am", "i'm", "i love", "i hate", "my name",
                    "i work", "my goal", "i want to", "i need"]
    low_signals  = ["okay", "thanks", "sure", "got it", "hello", "hi"]
